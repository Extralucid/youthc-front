...
export default function Step2({ plans }: Step2Props) {
  ...
  return (
    <div className="flex flex-col">
      <h1 className="my-3 lg:mt-8">Choisissez votre pack</h1>
      <p className="mb-5 lg:mb-10">
        Vous avez des options de paiement mensuel et annuel.
      </p>
      ...
      <div className="grid grid-cols-1 grid-rows-3 gap-5 lg:grid-cols-3 lg:grid-rows-1">
        {plans.map((plan) => (
          ...
          >
            <img
              src={`/assets/img/${plan.icon}`}
              alt={`${plan.name} icon`}
              className={`${watch("billing") === "annuel" && "place-self-start"} h-10 md:h-12 lg:h-14 lg:place-self-start`}
            />
