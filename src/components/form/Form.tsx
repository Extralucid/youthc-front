...
const steps: Steps = [
  ...
  { id: "step-1", name: "Vos infos", fields: ["name", "email", "phone"] },
  { id: "step-2", name: "Packs", fields: ["plan"] },
  { id: "step-3", name: "Garanties", fields: ["billing"] },
  { id: "step-4", name: "Resumé", fields: ["addons"] },
];
...
const createPlan = (name: FormValues["plan"], price: number): Plan => ({
  ...
  yearlyPrice: `+${formatCurrency(price * 10)}/an`,
});

const plans: Plans = [
  createPlan("simple", 9000),
  createPlan("avancé", 12000),
  createPlan("personnalisé", 15000),
];
...
const addons: Addons = [
  createAddon(
    ...
    "Pprise en charge des blessures et degats",
    1000,
  ),
  createAddon("vehicule", "dommages occasionnés au véhicule", "garantie dommages collision", 2000),
  createAddon(
    "conducteur",
