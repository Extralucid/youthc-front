...
import TablePagination from "../../../components/blog-category-table/TablePagination";
import Swal from "sweetalert2";
import axios from "axios";
import { listBlogCategorys } from "../../../slices/blog-category.slice";
...
const BlogCategory: React.FC = () => {
    ...
    const fetchData = useCallback(async (pageSize: number, pageIndex: number, search: string, order: any[]) => {
        ...
        dispatch(listBlogCategorys(queryOptions)).unwrap()
            .then((resp) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    setData((prevData) => prevData.filter((item) => item.category_id !== id));
                    Swal.fire({
                        title: "Supprimé!",
                        text: "BlogCategory supprimé avec succès!",
                        icon: "success",
                    });
                } catch (error) {
    ...
    const columns = useMemo(() => [
        {
            ...
            Cell: ({ row }: { row: { original: BlogCategory } }) => `R#${row.original.category_id}`,
            disableSortBy: true,
        },
        {
            Header: "Categorie",
        ...
        },
        {
            Header: "Slug",
            accessor: "slug",
            Cell: ({ row }: { row: { original: BlogCategory } }) => row.original.slug,
        },
        {
            Header: "Actions",
            Cell: ({ row }: { row: { original: BlogCategory } }) => {
                return (
                    <div className="flex gap-2">
                        <IconButton size="xs" color="green" appearance="primary" onClick={() => handleDelete(row.original.category_id)} icon={<Eye size="1.2em"/>} />
                        &nbsp;
                        <IconButton size="xs" color="orange" appearance="primary" onClick={() => handleDelete(row.original.category_id)} icon={<Edit size="1.2em"/>} />
                        &nbsp;
                        <IconButton size="xs" color="red" appearance="primary" onClick={() => handleDelete(row.original.category_id)} icon={<Trash2 size="1.2em"/>} />
                    </div>
                );
            },
        },
