...
import TablePagination from "../../../components/skill-table/TablePagination";
import Swal from "sweetalert2";
import axios from "axios";
import { listSkills } from "../../../slices/skill.slice";
...
const Skill: React.FC = () => {
    ...
    const fetchData = useCallback(async (pageSize: number, pageIndex: number, search: string, order: any[]) => {
        ...
        dispatch(listSkills(queryOptions)).unwrap()
            .then((resp) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    setData((prevData) => prevData.filter((item) => item.skill_id !== id));
                    Swal.fire({
                        title: "Supprimé!",
    ...
    const columns = useMemo(() => [
        {
            Header: "ID",
            accessor: "skill_id",
            Cell: ({ row }: { row: { original: Skill } }) => `R#${row.original.skill_id}`,
            disableSortBy: true,
        },
        {
            Header: "Competence",
        ...
        },
        {
            Header: "Slug",
            accessor: "slug",
            Cell: ({ row }: { row: { original: Skill } }) => row.original.slug,
        },
        {
            Header: "Actions",
            Cell: ({ row }: { row: { original: Skill } }) => {
                return (
                    <div className="flex gap-2">
                        <IconButton size="xs" color="green" appearance="primary" onClick={() => handleDelete(row.original.skill_id)} icon={<Eye size="1.2em"/>} />
                        &nbsp;
                        <IconButton size="xs" color="orange" appearance="primary" onClick={() => handleDelete(row.original.skill_id)} icon={<Edit size="1.2em"/>} />
                        &nbsp;
                        <IconButton size="xs" color="red" appearance="primary" onClick={() => handleDelete(row.original.skill_id)} icon={<Trash2 size="1.2em"/>} />
                    </div>
                );
            },
        },
